if not global.pause and not framesUntilControlable > 0 {
	...
	var move = key_right - key_left
		...
		if (place_meeting(x,y+1,oWall) or place_meeting(x,y+1,oBigMushroom)) and (key_jump)
		{
			vsp = -9
		}
		if place_meeting(x,y+1,oBigMushroom) {
			if oBigMushroom.bounceFrame > 0 {
				vsp = -35
			}
		}
		...
		if not noClip {
		if (place_meeting(x+hsp,y,oWall) or place_meeting(x+hsp,y,oBigMushroom) or place_meeting(x+hsp,y,oEnemyBarrier) or place_meeting(x+hsp,y,oTargetDoor) or place_meeting(x+hsp,y,oWallJumpDoor) or place_meeting(x+hsp,y,oShieldBreakDoor)) 
		{
	
			repeat(8)
			{
				if (place_meeting(x+hsp, y, oWall) or place_meeting(x+hsp,y,oBigMushroom) or place_meeting(x+hsp,y,oEnemyBarrier) or place_meeting(x+hsp,y,oTargetDoor) or place_meeting(x+hsp,y,oWallJumpDoor) or place_meeting(x+hsp,y,oShieldBreakDoor)) y -= 1;
			}
	
			if (place_meeting(x+hsp, y, oWall) or place_meeting(x+hsp,y,oBigMushroom) or place_meeting(x+hsp,y,oEnemyBarrier) or place_meeting(x+hsp,y,oTargetDoor) or place_meeting(x+hsp,y,oWallJumpDoor) or place_meeting(x+hsp,y,oShieldBreakDoor))
			{
				while (!place_meeting(x+sign(hsp),y,oWall) and !place_meeting(x+sign(hsp),y,oBigMushroom) and !place_meeting(x+sign(hsp),y,oEnemyBarrier) and !place_meeting(x+sign(hsp),y,oTargetDoor)  and !place_meeting(x+sign(hsp),y,oWallJumpDoor)  and !place_meeting(x+sign(hsp),y,oShieldBreakDoor))
				{
					x = x + sign(hsp);
				}
		...
		if not noClip or vsp >= 0 and not (key_down and godMode) {
			if (place_meeting(x,y+vsp,oWall) or place_meeting(x,y+vsp,oBigMushroom))
			{
				while (!place_meeting(x,y+sign(vsp),oWall) and !place_meeting(x,y+sign(vsp),oBigMushroom))
				{
					y = y + sign(vsp)
				}	
				vsp = 0
	...
	//Animation
	if (!place_meeting(x,y+1,oWall) and !place_meeting(x,y+1,oBigMushroom))
	{
		switch(state) {
			case("free"):
				if cling {
